...
curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $token" -sSk "$keycloak_url/auth/admin/realms/applications/clients" -d '{"clientId": "httpbin-two", "name": "httpbin-two", "enabled": true, "baseUrl": "https://httpbin-two.localhost", "redirectUris": ["https://httpbin-two.localhost/*"]}' > /dev/null
