...
# Sets up a ace client if it does not exist, then
# Writes ace's client_secret to stdout
...
curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $token" -sSk "$keycloak_url/auth/admin/realms/applications/clients" -d '{"clientId": "ace", "name": "ace", "enabled": true, "baseUrl": "https://ace.localhost", "redirectUris": ["https://ace.localhost/*"]}' > /dev/null
...
# Get ace's id
client_id=$(curl -X GET -H "Content-Type: application/json" -H "Authorization: Bearer $token" -sSk "$keycloak_url/auth/admin/realms/applications/clients" | jq -r '.[] | select(.clientId == "ace") | .id')

# Add the Audience mapper for this client (aud claim should read ace)
curl -H "Content-Type: application/json" -H "Authorization: Bearer $token" -sSk "$keycloak_url/auth/admin/realms/applications/clients/$client_id/protocol-mappers/models" --data-binary '{"protocol":"openid-connect","config":{"id.token.claim":"false","access.token.claim":"true","included.client.audience":"ace"},"name":"ace-aud","protocolMapper":"oidc-audience-mapper"}' > /dev/null
