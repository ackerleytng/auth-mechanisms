...
curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $token" -sSk "$keycloak_url/auth/admin/realms/applications/clients" -d '{"clientId": "httpbin", "name": "httpbin", "enabled": true, "baseUrl": "https://httpbin.test:7443", "redirectUris": ["https://httpbin.test:7443/*"]}' > /dev/null
