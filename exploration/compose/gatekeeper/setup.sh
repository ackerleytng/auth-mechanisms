...
curl -X POST -H "Content-Type: application/json" -H "Authorization: Bearer $token" -sSk "$keycloak_url/auth/admin/realms/applications/clients" -d '{"clientId": "httpbin", "name": "httpbin", "enabled": true, "baseUrl": "https://httpbin.localhost", "redirectUris": ["https://httpbin.localhost/*"]}' > /dev/null
